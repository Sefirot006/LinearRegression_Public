...
LogRegression::train(const Trainning_Set& train_set){
  // Coste anterior.
	...
	file.open("exit.csv");
	do{
		coste = cost(train_set);
    	gradientDescent(temp, alphaDivNum, file, k, train_set);
    	k++;
    	//cout << "k: " << k << endl;
  }while(k<300000 && convergence(c0, coste, train_set));
...
LogRegression::evaluate(const std::vector<double>& x) const{
	...
	//cout << "antes de la sigmoid: " << h << endl;
	h = sigmoid(h);
	//cout << "despues de la sigmoid: " << h << endl;

	return h;
}

bool
LogRegression::convergence(double& c0, double& coste, const Trainning_Set& train_set) const{
  //double c = 0.0;
  //c = cost(train_set);
  //cout << "c: " << coste << endl;
  //cout << "c0: " << c0 << endl;
  //cout << "valor de convergencia: " << std::abs(coste-c0) << endl;
  bool r = std::abs(coste-c0)>0.0001;
  c0 = coste;
  return r;
}
/*double
...
LogRegression::gradientDescent(double *temp, double alphaDivNum, std::ofstream& file, unsigned k, const Trainning_Set& train_set){
  ...
  unsigned m = train_set.size();
	for(i=0;i<=dim;++i){
		...
		temp[i] = temp[i]/train_set.get_dim()+1;
		//cout<< "i: " << i << endl;
